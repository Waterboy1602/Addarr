...
logger = logger.getLogger("addarr.sportarr", logLevel, config.get("logToConsole", False))

config = config["sportarr"]
...
def search(title):
    ...
    req = requests.get(commons.generateApiQuery("sportarr", "series/lookup", parameters))
...
def inLibrary(tvdbId):
    parameters = {}
    req = requests.get(commons.generateApiQuery("sportarr", "series", parameters))
    parsed_json = json.loads(req.text)
...
def addToLibrary(tvdbId, path, qualityProfileId, tags, seasonsSelected):
    ...
    add = requests.post(commons.generateApiQuery("sportarr", "series"), data=data, headers={'Content-Type': 'application/json'})
...
def removeFromLibrary(tvdbId):
    ...
    delete = requests.delete(commons.generateApiQuery("sportarr", f"series/{dbId}", parameters))
...
def getRootFolders():
    parameters = {}
    req = requests.get(commons.generateApiQuery("sportarr", "Rootfolder", parameters))
    parsed_json = json.loads(req.text)
...
def getQualityProfiles():
    parameters = {}
    req = requests.get(commons.generateApiQuery("sportarr", "qualityProfile", parameters))
    parsed_json = json.loads(req.text)
    return parsed_json
...
def getTags():
    parameters = {}
    req = requests.get(commons.generateApiQuery("sportarr", "tag", parameters))
    parsed_json = json.loads(req.text)
    return parsed_json
...
def createTag(tag):
    ...
    add = requests.post(commons.generateApiQuery("sportarr", "tag"), json=data_json, headers={'Content-Type': 'application/json'})
...
def getDbIdFromImdbId(tvdbId):
    req = requests.get(commons.generateApiQuery("sportarr", "series", {}))
    parsed_json = json.loads(req.text)
